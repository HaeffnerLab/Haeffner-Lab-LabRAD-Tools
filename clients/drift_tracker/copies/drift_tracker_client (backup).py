...
class drift_tracker(QtWidgets.QWidget):
    ...
    def create_widget_layout(self):
        ...
        keep_B_layout = QtWidgets.QHBoxLayout()
        keep_B_layout.addWidget(QtWidgets.QLabel("Tracking Duration (B)"))
        keep_B_layout.addWidget(self.track_B_duration)

        keep_line_center_layout = QtWidgets.QHBoxLayout()
        keep_line_center_layout.addWidget(QtWidgets.QLabel("Tracking Duration (Line Center)"))
    ...
    def on_new_B_track_duration(self, value):
        ...
        yield server.history_duration_local((rate_B, rate_line_center), client_name)
    ...
    def update_fit(self, bool_center_global = False):
        try:
            server = yield self.cxn.get_server('SD Tracker Global')
            client_list = yield server.get_clients()
            ...
            if bool_center_global:
                fit_f = yield server.get_fit_parameters_global('linecenter', client_name)
            else:
