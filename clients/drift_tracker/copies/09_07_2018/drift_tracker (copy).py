...
colors = default_color_cycle[0:len(client_list)]
...
class drift_tracker(QtWidgets.QWidget):
    ...
    def create_widget_layout(self):
        ...
        layout.addWidget(self.global_checkbox, 7, 1, 1, 1)
    ...
    def on_remove_all_B_and_line_centers(self, clicked):
        ...
        try:
            ...
            for k in range(len(line_center) - 1):
                yield server.remove_line_center_measurement(0, client_name)
        except self.Error or ValueError as e:
            self.displayError(e.msg)
    ...
    def update_track(self, meas, axes, lines):
        ...
        if (type(fitted)) == dict:
            ...
            if self.global_checkbox.isChecked():
                try:
                    ...
                    x_fit_data = [m[0] for m in fit_data]
                    y_fit_data = [m[1] for m in fit_data]
                    xmin = numpy.amin(x_fit_data)
                    xmax = numpy.amax(x_fit_data)
                    ymin = numpy.amin(y_fit_data)
                    ymax = numpy.amax(y_fit_data)
                except ValueError:
                    QtWidgets.QErrorMessage.qtHandler().showMessage('Global fit data points is none. You must include at least one data point.')
                    raise Exception("Global fit data points is none. You must include at least one data point.")
            else:
                try:
                    ...
                    QtWidgets.QErrorMessage.qtHandler().showMessage('Local line center fit data points is none. You must add at least one data point.')
                    raise Exception("Local line center fit data points is none. You must add at least one data point.")
            if xmin == xmax:
                xlims = [xmin-5,xmax+5]
                ylims = [ymin-2,ymax+2]
            else:
        ...
        if (type(fitted)) == list:
            ...
            try:
                ...
                QtWidgets.QErrorMessage.qtHandler().showMessage('Local B field fit data points is none. You must add at least one data point.')
                raise Exception("Local B field fit data points is none. You must add at least one data point.")
            if xmin == xmax:
                xlims = [xmin-5,xmax+5]
                ylims = [ymin-2,ymax+2]
            else:
