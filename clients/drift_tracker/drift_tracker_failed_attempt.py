...
class drift_tracker(QtGui.QWidget):
    ...
    def update_track(self, meas, axes, lines):
        ...
        try:
            ...
            raise Exception("Cannot reach to server")
        else:
            if (type(fitted)) == dict:
                ...
                if self.global_checkbox.isChecked():
                    ...
                    xmin = numpy.amin(numpy.array([m[0] for m in fitted[client]]))
                    xmax = numpy.amax(numpy.array([m[0] for m in fitted[client]]))
                    ymin = numpy.amin(numpy.array([m[1] for m in fitted[client]]))
                    ymax = numpy.amax(numpy.array([m[1] for m in fitted[client]]))
                if xmin == xmax:
                    xlims = [xmin-5,xmax+5]
                    ylims = [ymin-2,ymax+2]
                else:
            ...
            if (type(fitted)) == list:
                ...
                line_nofit = axes.plot(xnofit,ynofit, 'o', color = colors[client_list.index(client_name)], label = client_name)[0]
                legend = axes.legend()
                lines.append(line)
                lines.append(line_nofit)
                lines.append(legend)
