...
class readout_histogram_copy(QtGui.QWidget):
    def __init__(self, reactor, cxn = None, parent=None):
        ...
        self.last_hist = None
        self.last_plot = None
        self.subscribed = [False, False, False]
        self.pmtupdates = 0
    ...
    def subscribe_pulser(self):
        ...
        yield pulser.signal__sequence_programmed(config_729_hist.ID_C, context = self.context)
        yield pulser.addListener(listener = self.on_new_sequence, source = None, ID = config_729_hist.ID_C, context = self.context)
        self.subscribed[2] = True
    ...
    def reinitialize_pulser(self):
        ...
        yield server.signal__sequence_programmed(config_729_hist.ID_C, context = self.context)
        if not self.subscribed[2]:
            yield server.addListener(listener = self.on_new_sequence, source = None, ID = config_729_hist.ID_C, context = self.context)
            self.subscribed[2] = True
    ...
    def on_new_seq(self, dds, ttl, channels):
        ...
        dds_729v = [el for el in dds if el[0] == '729vert']
        dds_866 = [el for el in dds if el[0] == '866DP']

        # if self.last_hist is not None:
        #     self.axes.cla()
        #     #explicitly delete the refrence although not necessary
        #     del self.last_hist
        # # self.last_hist = self.axes.plot([el[1] for el in dds_729v if el[1]])

        # freqs = [el[1] for el in dds_729v if el[1]]
        ...
        toplot = [el[1] for el in dds_866 if el[1]]
        # print(dds_729v)
        # print(toplot)
        self.last_plot = self.axes.plot(toplot)
        # self.last_plot = self.axes.plot(range(5), range(5))
        #redo the limits
        # x_maximum = data[:,0].max()
        # self.axes.set_xlim(left = 0)
        # if x_maximum > self.axes.get_xlim()[1]:
        ...
        # # self.axes.plot([el[2] for el in dds_729v if el[1]])
        # self.canvas.draw()
        self.update_title()

    def update_title(self):
        self.axes.set_title(str(self.pulserupdates) + ', ' + str(self.pmtupdates))
        self.canvas.draw()
