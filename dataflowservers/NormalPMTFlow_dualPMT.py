...
class PMT(object):
    def __init__(self, pmtID):
        self.id = pmtID
        self.suffix = ' ' + str(pmtID+1)
        self.recording = False
        self.context = None

    def record(self, state):
        if state: self.recording = True
...
class NormalPMTFlow( LabradServer):
    ...
    # onNewSetting2 = Signal(SIGNALID+11, 'signal: new setting 2', '(ss)')
    onNewState = Signal(SIGNALID+2, 'signal: new state', '(ss)')
    # onNewState = Signal(SIGNALID+12, 'signal: new state 2', '(ss)')
    ...
    def initServer(self):
        ...
        pmt1 = PMT(0)
        self.pmt2 = pmt2 = PMT(1)
        self.pmtList = [pmt1]
        if self.secondPMT: self.pmtList.append(pmt2)
    ...
    def connect_pulser(self):
        try:
            self.pulser = yield self.client.pulser_dev
            self.collectTimeRange = yield self.pulser.get_collection_time()
            self.secondPMT = yield self.pulser.get_second_pmt()
    ...
    def makeNewDataSet(self, folder, name):
        ...
        for pmt in self.pmtList:
            ...
            print newSet
        # if self.secondPMT:
        #     yield self.dv.new(name + ' 2', [('t', 'num')], [('KiloCounts/sec','866 ON','num'),('KiloCounts/sec','866 OFF','num'),('KiloCounts/sec','Differential Signal','num')], context = PMT2_context)
    ...
    @setting(12, 'Enable PMT', returns = '')
    def enablePMT(self, c, pmt, state):
        self.pmtList[pmt].recording = state
    ...
    @inlineCallbacks
    def getPMTCounts(self, pmt):
        if pmt: self.rawdata = yield self.pulser.get_secondary_pmt_counts()
        else: self.rawdata = yield self.pulser.get_pmt_counts()
    
    def processSignals(self, data, id):
        lastPt = data[-1]
        NormalCount = lastPt[1]
        if id: self.onNewCount2(NormalCount)
