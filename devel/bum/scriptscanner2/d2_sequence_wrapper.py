...
class d2_sequence_wrapper(pulse_sequence_wrapper):
    
    def __init__(self, module, sc, cxn):
        super(d2_sequence_wrapper,self).__init__( module, sc, cxn)
    ...
    def run(self, ident):
        ...
        self.parameter_to_scan, self.scan_unit, self.scan, self.submit_unit, self.scan_submit = self.scans['2d']
        try:
            self.window = self.module.scannable_params[self.parameter_to_scan][1]
        except:
            self.window = 'current'    
        ...
        for scan_param1 in self.scans['1d'][2]:
            ...
            self.run_single(self.module, scan_param1[self.submit_unit_1d])
    ...
    def run_single(self, module, d1_param):
        ...
        processed_result = module.run_finally(cxn, self.parameters_dict, np.array(data), np.array(data_x))
        submission_1d = [d1_param, processed_result]
        self.dv.cd(self.directory_1d, context = self.data_save_context_1d)
        self.dv.open_appendable(self.ds_1d[1], context = self.data_save_context_1d)
        self.dv.add(submission_1d, context = self.data_save_context_1d)
        self._finalize_single(cxn)
