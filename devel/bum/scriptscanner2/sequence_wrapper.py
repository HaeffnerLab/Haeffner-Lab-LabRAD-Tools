...
#        print "UPDATING"
        ...
        try:
            cxn_global = labrad.connect('192.168.169.86', password = 'lab', tls_mode = 'off')
        except:
            print "Cannot connect to Global SD Tracker"
        else:
            try:
                carriers = cxn_global.sd_tracker_global.get_current_lines(cl.client_name)
            except:
                print "Cannot get current lines from Global SD Tracker"
            else:
                for c, f in carriers:
                    carriers_dict[carrier_translation[c]] = f
            cxn_global.disconnect()
