...
class SDTrackerGlobal(LabradServer):
    ...
    @setting(3, "Get Clients", returns = '*s')
    ...
    @setting(31, "Get Fit Line Center Global", client = 's', returns = '*v')
...
##            raise Exception("Requested line not found")
    ...
    def do_fit(self, client):
        ...
        for key in client_list:
            ...
            if (len(t_measure_line_center_global)):
                self.line_center_fit_global[key] = self.fitter.fit(t_measure_line_center_global, line_center_global)
        self.onNewFit(None)
