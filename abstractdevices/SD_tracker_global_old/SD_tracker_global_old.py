...
keep_B_measurements_global = dict.fromkeys(client_list, 100*60)
...
class SDTrackerGlobal(LabradServer):
    ...
    def initServer(self):
        ...
        self.keep_B_measurements_global = keep_B_measurements_global
        ...
        self.B_fit_global = dict.fromkeys(client_list)
    ...
    @setting(15, 'Set Measurements with Bfield and Line Center', B = '*(sv[gauss])', freq = '*(sv[MHz])', client = 's', returns = '')
    ...
    @setting(30, "Get Fit Parameters Local", name = 's', client = 's', returns = '*v')
    ...
    @setting(31, "Get Fit Parameters Global", name = 's', client = 's', returns = '*v')
    def get_fit_parameters_global(self, c, name, client):
        '''returns the parameters for the latest fit, name can be linecenter or bfield'''
        if name == 'linecenter':
            fit = self.line_center_fit_global[client]
        elif name =='bfield':
            fit = self.B_fit_global[client]
        else:
            raise Exception("Provided name or client not found")
        if fit is not None:
            return fit
    ...
    def get_current_lines_global(self, c, client):
        ...
        current_time = time.time() - self.start_time
        try:
            B = self.fitter.evaluate(current_time, self.B_fit_global[client])
...
##            raise Exception("Requested line not found")
    ...
    @setting(61, "Get Current B Global", client = 's', returns = 'v[gauss]')
    def get_current_b_global(self, c, client):
        current_time = time.time() - self.start_time
        B = self.fitter.evaluate(current_time, self.B_fit_global[client])
        B = WithUnit(B, 'gauss')
        return B
    ...
    @setting(81, 'History Duration Global', client = 's', duration = '*v[s]', returns = '*v[s]')
    def get_history_duration_Global(self, c, client, duration = None):
        if duration is not None:
            self.keep_B_measurements_global[client] = duration[0]['s']
            self.keep_line_center_measurements_global[client] = duration[1]['s']
        return [ WithUnit(self.keep_B_measurements_global[client],'s'), WithUnit(self.keep_line_center_measurements_global[client], 's') ]
    ...
    def do_fit(self):
        ...
        for client in client_list:
            ...
            if (len(self.t_measure_B[client])) and (len(self.t_measure_line_center[client])):
        ...
        t_measure_line_center_global = self.arraydict_join(self.t_measure_line_center)
        for client in client_list:
            keep_B = numpy.where((self.current_time - t_measure_B_global) < self.keep_B_measurements_global[client])
            ...
            B_field_global = B_field_global[keep_B]
            t_measure_B_global = t_measure_B_global[keep_B]
            line_center_global = line_center_global[keep_line_center]
            t_measure_line_center_global = t_measure_line_center_global[keep_line_center]
            self.B_fit_global[client] = self.fitter.fit(t_measure_B_global, B_field_global)
            self.line_center_fit_global[client] = self.fitter.fit(t_measure_line_center_global, line_center_global)
        self.onNewFit(None)
